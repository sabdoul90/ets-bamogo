...
export default function EditerPage() {
    ...
    const decodedId = rawId ? Number(decodeURIComponent(rawId)) : undefined;
    const venteId = decodedId;
    const isValidId = typeof venteId === "number" && !Number.isNaN(venteId) && venteId > 0;
    ...
    const editerVente = async () => {
        ...
        try {
            ...
            // Update vente
            const venteRequeste = await venteService.update(venteId!, {
                "montant": total,
    ...
    const recupVente = useCallback(async () => {
        ...
        if (!isValidId || loadingVenteRef.current) return;
    ...
    if (!isValidId) {
        return (
            <div className="p-6 text-center text-(--erreur)">
